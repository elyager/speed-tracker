...
if(staticMode){let settingsForm=document.getElementById('frequency-settings');settingsForm.querySelector('input').disabled=true;settingsForm.querySelector('button').disabled=true;document.getElementById('settings-output').textContent='Editable in the local dashboard';let payload=await (await fetch('./data.json',{cache:'no-store'})).json(),all=payload.runs||[],hours={'24h':24,'7d':168,'30d':720}[selected],cutoff=hours?Date.now()-hours*3600000:0;runs=all.filter(r=>new Date(r.started_at).getTime()>=cutoff).reverse();l=all[0]||null;h=payload.current_health||{}}
