...
<form id="frequency-settings" class="settings"><label>Test frequency (minutes)<input id="collection-interval" type="number" min="15" max="1440" step="1" required></label><button type="submit">Save</button><span id="settings-output" class="settings-output"></span></form>
<section class="cards">
...
<section class="panel wide"><h2>Recent tests</h2><table><thead><tr><th>Timestamp</th><th>Download</th><th>Upload</th><th>p95</th><th>p99</th><th>Packet loss</th><th>DNS</th><th>TCP</th><th>TLS</th><th>TTFB</th><th>Result</th></tr></thead><tbody id="rows"></tbody></table></section>
</main><script>
...
if(staticMode){let settingsForm=document.getElementById('frequency-settings');settingsForm.querySelector('input').disabled=true;settingsForm.querySelector('button').disabled=true;document.getElementById('settings-output').innerHTML='Editable in the local dashboard: <a href="http://127.0.0.1:8765/">http://127.0.0.1:8765/</a>';let payload=await (await fetch('./data.json',{cache:'no-store'})).json(),all=payload.runs||[],hours={'24h':24,'7d':168,'30d':720}[selected],cutoff=hours?Date.now()-hours*3600000:0;runs=all.filter(r=>new Date(r.started_at).getTime()>=cutoff).reverse();l=all[0]||null;h=payload.current_health||{}}
else{let settingsForm=document.getElementById('frequency-settings');settingsForm.hidden=false;fetch('/api/settings').then(response=>response.json()).then(settings=>{document.getElementById('collection-interval').value=settings.collection_interval_minutes}).catch(error=>{document.getElementById('settings-output').textContent='Could not load frequency'});let [sumRes,runsRes]=await Promise.all([fetch('/api/summary'),fetch('/api/runs?range='+selected+'&limit=10000')]),sum=await sumRes.json();runs=(await runsRes.json()).runs.reverse();l=sum.latest;h=sum.current_health||{}}
